...
import { apiClient, CreatePostRequest } from "@/lib/api"
...
export default function CreatePostPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.createPost(formData, selectedFiles)
      if (response.status === "success" && response.data?.post) {
        router.push(`/post/${response.data.post.id}`)
      }
    } catch (err) {
      setError(err instanceof Error ? err.message : "เกิดข้อผิดพลาดในการสร้างโพสต์")
    } finally {
          ...
          <FadeUp>
            <div className="max-w-3xl mx-auto">
              <div className="mb-6">
                ...
                <h2 className="text-3xl font-bold tracking-tight">สร้างโพสต์ใหม่</h2>
                <p className="text-muted-foreground mt-1">
                  กรอกข้อมูลเพื่อสร้างโพสต์ใหม่
                </p>
              </div>
                ...
                >
                  <form onSubmit={handleSubmit}>
                    ...
                    <CardContent className="px-8 py-4 space-y-4">
                      ...
                      {/* Image Upload */}
                      <div className="grid gap-2">
                        <Label htmlFor="images">รูปภาพ (สูงสุด 10 รูป)</Label>
                        <div className="space-y-4">
                          ...
                          {selectedFiles.length > 0 && (
                            <p className="text-xs text-muted-foreground">
                              อัปโหลดแล้ว {selectedFiles.length} / 10 รูป
                            </p>
                          )}
                        </div>
                      </div>
